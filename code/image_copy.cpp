...
void edge_enhance();
void rgb2bmp(char *);
void changeVibrance(double change);
void getColor();
...
void adjustContrast(double contrast) {
  ...
  int i, j;
  // Map the contrast from [-1, 1] to [-255, 255]
  double mappedContrast = 255 * contrast;
...
void changeVibrance(double vibrance_factor) {
  vibrance_factor *= -1;
  for (int i = 1; i < 2459; i++) // Height
  {
    for (int j = 1; j < 3359; j++) // Width
    {
      ...
      double avg = (r[i][j] + g[i][j] + b[i][j]) / 3;

      double amt = ((abs(max - avg * 2) * 2) * vibrance_factor) / 100;

      r[i][j] = r[i][j] + (max - r[i][j]) * amt;
      g[i][j] = g[i][j] + (max - g[i][j]) * amt;
      b[i][j] = b[i][j] + (max - b[i][j]) * amt;
    }
  }
}
