...
//			b_hist[(pic[i][j+1]/2)]++;	// E
	...
	// �ƭȡ� �N���
	float Y_bright=0.0, Cr_bright=192.0, Cb_bright=192.0;  // probably white point 192 
...
void edge_enhance()
{
	for(int i=0; i<2460; i++)
	{
		for(int j=5; j<3293; j++)
		{
			...
			{
				pic[i][j] = 2 * r[i][j] - (r[i-1][j] + r[i][j-1] + r[i][j+1] + r[i+1][j]) / 4;
			}
	...
	for(int i=0; i<2460; i++)
	{
		for(int j=5; j<3293; j++)
		{
			...
			{
				pic[i][j] = 2 * g[i][j] - (g[i-1][j] + g[i][j-1] + g[i][j+1] + g[i+1][j]) / 4;
			}
	...
	for(int i=0; i<2460; i++)
	{
		for(int j=5; j<3293; j++)
		{
			...
			{
				pic[i][j] = 2 * b[i][j] - (b[i-1][j] + b[i][j-1] + b[i][j+1] + b[i+1][j]) / 4;
			}
		}
	}
...
void CFA_to_krkb(){
	...
	for(i=1; i<2459; i=i+2)  // Height
	{
		...
		{
			r[i][j] = ( pic[i-1][j-1] + pic[i-1][j+1] + pic[i+1][j-1] + pic[i+1][j+1] ) / 4;
			g[i][j] = ( pic[i-1][j] + pic[i][j-1] + pic[i][j+1] + pic[i+1][j] ) / 4;
			b[i][j] = pic[i][j];
		}
	}
